...
void man(char *name)
{
	...
	{	
		printf("Description : Used to display information in file\n");
	...
	else if(strcmp(name,"open") == 0)
	{
		...
		printf("Usgae : open File_name mode\n");
	}
	else if(strcmp(name,"close") == 0)
	{
		printf("Description : Used to close opened file\n");
		printf("Usgae : close File_name\n");
	}
	else if(strcmp(name,"closeall") == 0)
	{
		printf("Description : Used to close all opened file\n");
		printf("Usgae : closeall\n");
	}
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode == READ) ||(UFDTArr[fd].ptrfiletable->mode == READ+WRITE))
	{
		...
		{
			if(size > (UFDTArr[fd].ptrfiletable->readoffset))					return -1;
			if(size < 0) return -1;
	...
	while(1)
	{
		...
		else if(count == 2)
		{
			if(strcmp(command[0],"stat") == 0)
			{
				...
				if(ret == -2)
				{
					printf("ERROR : There is so such file\n");
				}
				continue;
			}
